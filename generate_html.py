...
def process_directory(yaml_file, directory, output_folder="logs"):
    ...
    json_pattern = os.path.join(directory, "*_result.json")
    log_pattern = os.path.join(directory, "*_tc_result.log")
...
def main(yaml_file, json_file, log_file=None, output_folder="logs"):
    ...
    if log_file and not log_file.endswith("_tc_result.log"):
        logger.error(f"Invalid Log file format: '{log_file}'. It must end with '_tc_result.log'.")
        sys.exit(1)
